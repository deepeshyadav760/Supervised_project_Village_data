...
# Function for Decision Tree prediction
def dt_predict(data):
    categorical_vars = {
        'AVAILABILITY OF GOVERNMENT SEED CENTRES': ['Yes', 'No (Nearest facility < 1 km)', 'No (Nearest facility 1-2 kms)', 'No (Nearest facility 2-5 kms)', 'No (Nearest facility 5-10 kms)', 'No (Nearest facilityMore than 10 kms)'],
        'WHETHER THIS VILLAGE IS A PART OF THE WATERSHED DEVELOPMENT PROJECT': ['Yes', 'No'],
...
# Streamlit interface
st.title("Village Data Prediction")

st.header("Linear Regression Prediction")
linear_input_data = {
    'NUMBER OF SHGS FEDERATED INTO VILLAGE ORGANISATIONS (VOS)': st.number_input('NUMBER OF SHGS FEDERATED INTO VILLAGE ORGANISATIONS (VOS)', 0),
    'NUMBER OF SHGS WHICH ACCESSED BANK LOANS': st.number_input('NUMBER OF SHGS WHICH ACCESSED BANK LOANS', 0),
    'NUMBER OF BENEFICIARIES RECEIVING BENEFITS UNDER AAYUSHMAN BHARAT-PRADHAN MANTRI JAN AROGYA YOJANA OR ANY STATE GOVT HEALTH SCHEME': st.number_input('NUMBER OF BENEFICIARIES RECEIVING BENEFITS UNDER AAYUSHMAN BHARAT-PRADHAN MANTRI JAN AROGYA YOJANA OR ANY STATE GOVT HEALTH SCHEME', 0),
    'TOTAL NUMBER OF HOUSEHOLDS RECEIVING FOOD GRAINS FROM FAIR PRICE SHOPS ': st.number_input('TOTAL NUMBER OF HOUSEHOLDS RECEIVING FOOD GRAINS FROM FAIR PRICE SHOPS ', 0),
    'TOTAL NUMBER OF FARMERS ': st.number_input('TOTAL NUMBER OF FARMERS ', 0),
    'TOTAL EXPENDITURE APPROVED UNDER NRM IN THE LABOUR BUDGET FOR THE YEAR 2018-19)': st.number_input('TOTAL EXPENDITURE APPROVED UNDER NRM IN THE LABOUR BUDGET FOR THE YEAR 2018-19)', 0),
}
if st.button("Predict using Linear Regression"):
    linear_prediction = linear_predict(linear_input_data)
    st.write(f"Linear Regression Prediction: {linear_prediction}")

st.header("Decision Tree Prediction")
dt_input_data = {}
for var in [
    'NUMBER OF TOTAL POPULATION', 'NUMBER OF HOUSEHOLDS ENGAGED MAJORLY IN FARM ACTIVITIES', 
    'NUMBER OF HOUSEHOLDS ENGAGED MAJORLY IN NON-FARM ACTIVITIES', 'TOTAL AREA IRRIGATED (IN HECTARE)', 
    'TOTAL UNIRRIGATED LAND AREA (IN HECTARES)', 'AVAILABILITY OF HIGHER/SENIOR SECONDARY SCHOOL', 
    'NUMBER OF HOUSEHOLDS MOBILIZED INTO SHGS', 'NUMBER OF HOUSEHOLDS HAVING PIPED WATER CONNECTION']:
    dt_input_data[var] = st.number_input(var, 0)

for var, choices in categorical_vars.items():
    dt_input_data[var] = st.selectbox(var, choices)

if st.button("Predict using Decision Tree"):
    dt_prediction = dt_predict(dt_input_data)
